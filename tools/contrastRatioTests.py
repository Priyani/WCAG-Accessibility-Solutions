'''
Author: grimley517 (Brian Jones)
Date: January 2015

This is a series of tests to check coverage of contrastRatio.py's
library functions.
'''
    ...
    def test1(self):
        ...
        for i in range(len(trueIns)):
            self.assertEqual(cr.componentise(trueIns[i]), answers[i],
                             msg="Something went wrong with the {0}th item"
                             .format(i+1))
    ...
    def test2(self):
        ...
        for i in range(len(inputs)):
            self.assertEqual(cr.hexVal(inputs[i]), i,
                             msg="{0} converted to {1}".format(inputs[i],
                             cr.hexVal(inputs[i])))

    def test3(self):
        ''' tests the unitise funtion'''
        self.assertAlmostEqual(cr.unitise(255), 1.0,
                               msg="255 does not unitise to 1")

        self.assertAlmostEqual(cr.unitise(0), 0.0,
                               msg="0 does not unitise to 0")

        self.assertAlmostEqual(cr.unitise(51), 0.2,
                               msg="51 does not unitise to 0.2")

    def test4(self):
        '''tests the final contrast ratios are within the bounds,
        theoretical bounds are 1.0 to 21.0'''
        background = "#000000"
        for foreground in range(2 ^ 24):
            foreground = hex(foreground)
            foreground = "#" + foreground[2:]

            self.assertGreaterEqual(cr.ratio(background, foreground), 1.0,
                                    msg="{0} has a Contrast ratio of less than 1"
                                    .format(foreground))

            self.assertLessEqual(cr.ratio(background, foreground), 21.0,
                                 msg="{0} has a Contrast ratio of greater than 21"
                                 .format(foreground))

    def test5(self):
        '''Reference value test.
        Checks relative luminance against 15 calculated values'''
        tColours = ['000', '111', '222',
                    '333', '444', '555',
                    '666', '777', '888',
                    '999', 'aaa', 'bbb',
                    'ccc', 'ddd', 'eee', 'fff'
        ...
        for i in range(len(tColours)):
            self.assertEqual(cr.relLuminance(tColours[i]), tExpected[i],
                             msg="{0} does not have the expected relative luminance {1}"
                             .format(tColours[i], tExpected[i]))
