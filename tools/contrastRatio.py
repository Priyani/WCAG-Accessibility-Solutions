...
This is a library that can be used to calculate the contrast ratio
of two colours presented in hex format.
'''
...
def componentise(hexValue):
    ...
    size = int(size / 3)
    if size > 2:
        raise ValueError('''hex value should be 6 or 3 characters long {0}
        is invalid'''.format(hexValue))
    if size < 1:
...
def unitise(component):
    '''Converts the component value to a float representing
    the colours relative luninance
...
def luminanceComponent(component):
    '''from an entered string representing a hex colour component
    returns the components luminance value as a float
    '''
...
def relLuminance(hexValue):
    '''Converts an integer into a relative luminance
...
if __name__ == "__main__":
    ...
    colourText = input('''Please enter the Hexadecimal code
    for the text colour you are intending to use.''')
    colourBG = input('''And please enter the Code
    for the fine colour that you wish for your background? ''')
    ...
    if result >= 4.5:
        ...
        print ('''thats an OK Contrast Ratio
        for large text (over 18 pt, or 14 pt Bold)''')
    else:
